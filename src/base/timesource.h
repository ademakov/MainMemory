/*
 * base/timesource.h - MainMemory time utilities.
 *
...
#ifndef BASE_TIMESOURCE_H
#define BASE_TIMESOURCE_H
...
mm_timesource_prepare(struct mm_timesource *ts)
{
#if ENABLE_TIMESOURCE_TIMESTAMP
	ts->clock_stamp = 0;
...
static inline void NONNULL(1)
mm_timesource_refresh(struct mm_timesource *ts UNUSED)
{
#if ENABLE_TIMESOURCE_TIMESTAMP
	// Nothing to do.
#else
...
mm_timesource_gettime(struct mm_timesource *ts)
{
#if ENABLE_TIMESOURCE_TIMESTAMP
	uint64_t stamp = mm_cpu_timestamp();
	if ((ts->clock_stamp + MM_CLOCK_STAMP_DELTA) <= stamp)
	{
...
static inline mm_timeval_t NONNULL(1)
mm_timesource_getrealtime(struct mm_timesource *ts)
{
#if ENABLE_TIMESOURCE_TIMESTAMP
	uint64_t stamp = mm_cpu_timestamp();
	if ((ts->real_clock_stamp + MM_CLOCK_STAMP_DELTA) <= stamp)
	{
	...
	if (ts->real_clock_count) {
		...
		ts->real_clock_count = MM_CLOCK_COUNT;
		ts->real_clock_value = mm_clock_gettime_realtime_coarse();
		TRACE("%lld", (long long) ts->real_clock_value);
	}
#endif
